...
class RoomService:
    ...
    async def add_members_to_room(room_id: int, owner: User, usernames: list[str]) -> Room:
        ...
        if room.is_system:
            raise HTTPException(
                ...
                detail="Cannot add members to system room"
            )
    ...
    async def get_room(room_id: int, user: User) -> Room:
        ...
        room = await Room.filter(id=room_id).prefetch_related(
    ...
    async def leave_room(room_id: int, user: User) -> None:
        ...
        if room.is_system:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Cannot leave system room"
            )
    ...
    async def get_or_create_system_room(user1: User, user2: User) -> Room:
        ...
        if common_room_ids:
            ...
            system_room = await Room.filter(
                id__in=common_room_ids, is_system=True
            ).first()
